...
for (i in 1:n_obs) {
  ...
  # Relleno para TOA_other_moder_2
  if (is.na(TOA_other_moder_2[i]) && current_cfp_status %in% codes_other_moder_2) {
    TOA_other_moder_2[i] <- year_of_adoption_cfp_period
  }
}
...
TOA_derivado_orig <- read.csv("TOA_derivado_full.csv")
TOA_derivado_orig <- TOA_derivado_orig$TOA_derivado
